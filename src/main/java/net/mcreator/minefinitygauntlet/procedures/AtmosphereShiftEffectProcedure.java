...
public class AtmosphereShiftEffectProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != AtmosphereShiftEffectProcedure.amount || seed != AtmosphereShiftEffectProcedure.seed) {
			AtmosphereShiftEffectProcedure.amount = amount;
			AtmosphereShiftEffectProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
	...
	private static void execute(@Nullable Event event, LevelAccessor world, ResourceKey<Level> dimension, Entity entity) {
		...
		if (MinefinityGauntletModVariables.MapVariables.get(world).colorshift) {
			if (dimension == (entity.level().dimension())) {
				renderDeepSky(255 << 24 | 255 << 16 | 0 << 8 | 0);
			}
		}
	}
}
