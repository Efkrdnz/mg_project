...
public class P3BeamEffectProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (P3BeamEffectProcedure.bufferBuilder == null) {
			if (update)
				clear();
			if (P3BeamEffectProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					P3BeamEffectProcedure.mode = mode;
					P3BeamEffectProcedure.format = format;
					P3BeamEffectProcedure.bufferBuilder = Tesselator.getInstance().begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					P3BeamEffectProcedure.mode = mode;
					P3BeamEffectProcedure.format = format;
					P3BeamEffectProcedure.bufferBuilder = Tesselator.getInstance().begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		P3BeamEffectProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			P3BeamEffectProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
		...
		if (entity.getData(MinefinityGauntletModVariables.PLAYER_VARIABLES).beamPower) {
			...
			if (target(2)) {
				...
				renderShape(shape(), (entity.getX() + 0.2 * Math.cos(yaw) * Math.cos(pitch)), (entity.getY() + sneak + 0.2 * Math.sin((-1) * pitch)), (entity.getZ() + 0.2 * Math.sin(yaw) * Math.cos(pitch)), (float) (entity.getYRot() + 0.25),
						entity.getXRot(), 0, (float) (-1.6), (float) 1.6, 1, 32 << 24 | 191 << 16 | 64 << 8 | 191);
				renderShape(shape(), (entity.getX() + 0.2 * Math.cos(yaw) * Math.cos(pitch)), (entity.getY() + sneak + 0.2 * Math.sin((-1) * pitch)), (entity.getZ() + 0.2 * Math.sin(yaw) * Math.cos(pitch)), (float) (entity.getYRot() + 0.25),
						entity.getXRot(), 0, 1, 1, 1, 255 << 24 | 191 << 16 | 64 << 8 | 191);
				release();
			}
		}
	}
