...
public record SelectAbilityMindButtonMessage(int buttonID, int x, int y, int z, HashMap<String, String> textstate) implements CustomPacketPayload {

	public static final Type<SelectAbilityMindButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(MinefinityGauntletMod.MODID, "select_ability_mind_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, SelectAbilityMindButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, SelectAbilityMindButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
		writeTextState(message.textstate, buffer);
	}, (RegistryFriendlyByteBuf buffer) -> new SelectAbilityMindButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), readTextState(buffer)));
	@Override
	public Type<SelectAbilityMindButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final SelectAbilityMindButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z, HashMap<String, String> textstate) {
		...
		HashMap guistate = SelectAbilityMindMenu.guistate;
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		MinefinityGauntletMod.addNetworkMessage(SelectAbilityMindButtonMessage.TYPE, SelectAbilityMindButtonMessage.STREAM_CODEC, SelectAbilityMindButtonMessage::handleData);
	}
}
