...
public record SelectAbilityPowerButtonMessage(int buttonID, int x, int y, int z, HashMap<String, String> textstate) implements CustomPacketPayload {

	public static final Type<SelectAbilityPowerButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(MinefinityGauntletMod.MODID, "select_ability_power_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, SelectAbilityPowerButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, SelectAbilityPowerButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
		writeTextState(message.textstate, buffer);
	}, (RegistryFriendlyByteBuf buffer) -> new SelectAbilityPowerButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), readTextState(buffer)));
	@Override
	public Type<SelectAbilityPowerButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final SelectAbilityPowerButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z, HashMap<String, String> textstate) {
		...
		HashMap guistate = SelectAbilityPowerMenu.guistate;
		...
		if (buttonID == 0) {
			...
			AbilitySelectPower1Procedure.execute(entity);
		}
		if (buttonID == 1) {

			AbilitySelectPower2Procedure.execute(entity);
		}
		if (buttonID == 2) {

			AbilitySelectPower3Procedure.execute(entity);
		}
		if (buttonID == 3) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		MinefinityGauntletMod.addNetworkMessage(SelectAbilityPowerButtonMessage.TYPE, SelectAbilityPowerButtonMessage.STREAM_CODEC, SelectAbilityPowerButtonMessage::handleData);
	}
}
