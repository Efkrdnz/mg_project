...
import net.mcreator.minefinitygauntlet.procedures.AbilitySelectedSoul3Procedure;
import net.mcreator.minefinitygauntlet.procedures.AbilitySelectSoul5Procedure;
import net.mcreator.minefinitygauntlet.procedures.AbilitySelectSoul4Procedure;
import net.mcreator.minefinitygauntlet.procedures.AbilitySelectSoul2Procedure;
...
public record SelectAbilitySoulButtonMessage(int buttonID, int x, int y, int z, HashMap<String, String> textstate) implements CustomPacketPayload {

	public static final Type<SelectAbilitySoulButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(MinefinityGauntletMod.MODID, "select_ability_soul_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, SelectAbilitySoulButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, SelectAbilitySoulButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
		writeTextState(message.textstate, buffer);
	}, (RegistryFriendlyByteBuf buffer) -> new SelectAbilitySoulButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), readTextState(buffer)));
	@Override
	public Type<SelectAbilitySoulButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final SelectAbilitySoulButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z, HashMap<String, String> textstate) {
		...
		HashMap guistate = SelectAbilitySoulMenu.guistate;
		...
		if (buttonID == 7) {
			...
			AbilitySelectSoul2Procedure.execute(entity);
		}
		if (buttonID == 8) {

			AbilitySelectedSoul3Procedure.execute(entity);
		}
		if (buttonID == 9) {

			AbilitySelectSoul4Procedure.execute(entity);
		}
		if (buttonID == 10) {

			AbilitySelectSoul5Procedure.execute(entity);
		}
		if (buttonID == 11) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		MinefinityGauntletMod.addNetworkMessage(SelectAbilitySoulButtonMessage.TYPE, SelectAbilitySoulButtonMessage.STREAM_CODEC, SelectAbilitySoulButtonMessage::handleData);
	}
}
